...
const ShowMostPopularDes = () => {


    const axiosSecure = useAxiosSecure();
    // const {user}=useAuth();
       const {  data: mostPopDes=[], refetch } = useQuery({
           queryKey: [' mostPopDes'],
           queryFn: async() =>{
            const res = await axiosSecure.get('/show-popular_destination')
            return res.data}
  ...
  return (
    <div>
        <div>
            <h2 className='text-2xl font-medium font-playfair'> All Popular Destination : {mostPopDes.length}  </h2>
        </div>
  ...
  <table className="table ">
    ...
    <tbody>
      ...
      {
        mostPopDes.slice().reverse().map(( mostPop, indx)=>{
return <tr key={indx}>
        <th>{indx + 1 }</th>

        <td> <img className='w-10 hue-rotate-180' src={mostPop.image} alt="" /> </td>
        <td>{mostPop.name}</td>
       
        <td>{mostPop.country}</td>
...
export default ShowMostPopularDes;