...
const DomesticTicket = () => {
  ...
  useEffect(() => {
    fetch("https://world-holidays-backend-part-two.vercel.app/international_ticket")
      .then((res) => {
        if (!res.ok) {
      ...
      <section className=" mt-10 ">
        <div className="   md:flex justify-between gap-3 items-center ">
          ...
          {ticketData.map((ticket) => (
            <div key={ticket._id} className=" space-y-2 md:w-[35rem]   ">
              <h3 className="text-2xl font-semibold md:text-4xl font-playfair pb-4">
                {ticket.title}
              </h3>
              <p className="font-nunito font-light">{ticket.description}</p>
